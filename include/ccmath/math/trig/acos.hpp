...
namespace ccm
{
	 ...
	 */
	template <typename T, std::enable_if_t<!std::is_integral_v<T>, bool> = true> constexpr T acos(T num)
	{
		if constexpr (ccm::builtin::has_constexpr_acos<T>)
		{
			return ccm::builtin::acos_ct(num);
		} else if (ccm::support::is_constant_evaluated())
		{
			if constexpr (std::is_same_v<T, float>)
			{
				return internal::impl::acos_float(num);
			} else if constexpr (std::is_same_v<T, double>)
			{
				return internal::impl::acos_double(num);
			} else
			{
	 ...
	 */
	template <typename Integer, std::enable_if_t<std::is_integral_v<Integer>, bool> = true> constexpr double acos(Integer num)
	{
