...
namespace ccm
{
	template <typename T, std::enable_if_t<std::is_floating_point_v<T>, bool> = true> constexpr T nextdown(T x) noexcept
	{
		return ccm::nextafter(x, -std::numeric_limits<T>::infinity());
	}

	template <typename Integer, std::enable_if_t<std::is_integral_v<Integer>, bool> = true> constexpr double nextdown(Integer x) noexcept
	{
