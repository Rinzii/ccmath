...
namespace ccm::builtin
{
	...
	/**
	 * @internal
	 * Wrapper for constexpr __builtin_pow functions.
	 * This should be used internally and always be wrapped in an if constexpr statement.
	 * It exists only to allow for usage of __builtin_pow functions without triggering a compiler error
	 * when the compiler does not support them.
	 */
	template <typename T> constexpr auto pow_ct(T x, T y) -> std::enable_if_t<has_constexpr_pow<T>, T>
	{
	...
	template <typename T> auto pow_rt(T x, T y) -> std::enable_if_t<has_runtime_pow<T>, T>
	{
		if constexpr (std::is_same_v<T, float>)
		{
			return __builtin_powf(x, y);
