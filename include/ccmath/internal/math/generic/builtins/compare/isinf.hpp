...
namespace ccm::builtin
{
	...
	/**
	 * Wrapper for constexpr __builtin isinf functions.
	 * This should be used internally and always be wrapped in an if constexpr statement.
	 * It exists only to allow for usage of __builtin isinf functions without triggering a compiler error
	 * when the compiler does not support them.
	 */
	template <typename T>
	constexpr auto isinf(T x) -> std::enable_if_t<has_constexpr_isinf<T>, bool>
	{
		if constexpr (std::is_same_v<T, float>) { return __builtin_isinf(x); }
		else if constexpr (std::is_same_v<T, double>) { return __builtin_isinf(x); }
		else if constexpr (std::is_same_v<T, long double>) { return __builtin_isinf(x); }
