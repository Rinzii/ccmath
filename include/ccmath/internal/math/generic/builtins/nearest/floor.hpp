...
namespace ccm::builtin
{
	 ...
	 */
	template <typename T>
	inline constexpr bool has_constexpr_floor =
#ifdef CCMATH_HAS_CONSTEXPR_BUILTIN_FLOOR
	 ...
	 * It exists only to allow for usage of __builtin_floor functions without triggering a compiler error
	 * when the compiler does not support them.
	 */
	template <typename T> constexpr auto floor_ct(T x) -> std::enable_if_t<has_constexpr_floor<T>, T>
	{
	...
	template <typename T> auto floor_rt(T x) -> std::enable_if_t<has_runtime_floor<T>, T>
	{
		if constexpr (std::is_same_v<T, float>)
		{
			return __builtin_floorf(x);
