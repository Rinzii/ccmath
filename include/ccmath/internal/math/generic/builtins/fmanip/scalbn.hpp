...
namespace ccm::builtin
{
	 ...
	 */
	template <typename T>
	constexpr auto scalbn_ct(T x, int exp) -> std::enable_if_t<has_constexpr_scalbn<T>, T>
	{
	...
	template <typename T>
	auto scalbn_rt(T x, int exp) -> std::enable_if_t<has_runtime_scalbn<T>, T>
	{
		if constexpr (std::is_same_v<T, float>) { return __builtin_scalbnf(x, exp); }
		else if constexpr (std::is_same_v<T, double>) { return __builtin_scalbn(x, exp); }
		else if constexpr (std::is_same_v<T, long double>) { return __builtin_scalbnl(x, exp); }
	 ...
	 */
	template <typename T>
	constexpr auto scalbn_ct(T x, long exp) -> std::enable_if_t<has_constexpr_scalbn<T>, T>
	{
