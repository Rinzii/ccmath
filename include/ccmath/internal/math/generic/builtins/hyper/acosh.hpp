...
namespace ccm::builtin
{
	...
	/**
	 * @internal
	 * Wrapper for constexpr __builtin_acosh functions.
	 * This should be used internally and always be wrapped in an if constexpr statement.
	 * It exists only to allow for usage of __builtin_acosh functions without triggering a compiler error
	 * when the compiler does not support them.
	 */
	template <typename T> constexpr auto acosh_ct(T x) -> std::enable_if_t<has_constexpr_acosh<T>, T>
	{
	...
	template <typename T> auto acosh_rt(T x) -> std::enable_if_t<has_runtime_acosh<T>, T>
	{
		if constexpr (std::is_same_v<T, float>)
		{
			return __builtin_acoshf(x);
