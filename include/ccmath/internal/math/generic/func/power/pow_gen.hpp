...
namespace ccm::gen
{
	template <typename T, std::enable_if_t<std::is_floating_point_v<T>, bool> = true> constexpr T pow_gen(T base, T exp) noexcept
	{
		// TODO: Maybe add more specific optimizations for integers?
		if constexpr (std::is_same_v<T, float>)
		{
			return impl::powf_impl(base, exp);
		} else if constexpr (std::is_same_v<T, double>)
		{
			return impl::pow_impl(base, exp);
		} else if constexpr (std::is_same_v<T, long double>)
		{
			return powl_gen(base, exp);
		} else
		{
