...
namespace ccm::gen::impl
{
	namespace internal::impl
	{
		...
		constexpr double pow_kernel(double base, double exp, std::uint64_t sign) noexcept
		{
			...
			{
				const double c = FPBits_t(FPBits_t(m_x).uintval() & 0x3fff'e000'0000'0000ULL).get_val();
				dx			   = support::multiply_add(support::constants::RD.at(static_cast<std::size_t>(idx_x)), m_x - c,
													support::constants::CD.at(static_cast<std::size_t>(idx_x)));
			...
			const std::uint64_t exp2_hm_lo_i =
				idx_y != 0 ? static_cast<std::uint64_t>(exp2_hi_i + exp2_mid_lo_i) + sign : sign;
