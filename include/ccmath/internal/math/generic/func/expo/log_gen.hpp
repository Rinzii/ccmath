...
namespace ccm::gen
{
	...
	constexpr T log_gen(T num) noexcept
	{
		if constexpr (std::is_same_v<T, float>) { return ccm::internal::log_float(num); }
		if constexpr (std::is_same_v<T, double>) { return ccm::internal::log_double(num); }
		if constexpr (std::is_same_v<T, long double>) { return static_cast<long double>(ccm::internal::log_double(static_cast<double>(num))); }
