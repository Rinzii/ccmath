...
namespace ccm
{
	 ...
	 */
	template <typename T, std::enable_if_t<std::is_floating_point_v<T>, bool> = true> constexpr bool isless(T x, T y) noexcept
	{
		if constexpr (ccm::builtin::has_constexpr_isless<T>)
		{
			return ccm::builtin::isless_ct(x, y);
		} else
		{
