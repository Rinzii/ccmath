...
namespace ccm::pp
{
	template <typename T> struct SimdTraits<T, ScalarAbi>
	{
		using value_type						   = T;
		using SimdMember						   = T;
		...
		template <typename U> CCM_ALWAYS_INLINE static U convert(SimdMember v)
		{
			return static_cast<U>(v);
		}
		template <typename U> CCM_ALWAYS_INLINE static U bitcast(SimdMember v)
		{
			U out;
