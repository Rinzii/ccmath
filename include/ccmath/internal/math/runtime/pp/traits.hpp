...
namespace ccm::pp
{
	...
	template <>
	struct is_abi_tag<ScalarAbi> : std::true_type
	{
	};
	template <detail::SimdSizeType N>
	struct is_abi_tag<VecAbi<N>> : std::true_type
	{
	};
	template <typename T>
	...
	struct is_simd_mask<basic_simd_mask<Bytes, Abi>> : std::true_type
	{
	};
	template <typename T>
	...
	struct simd_size : std::integral_constant<detail::SimdSizeType, Abi::size>
	{
	};
	template <typename T, typename Abi = detail::NativeAbi<T>>
	...
	struct resize_simd;
	template <detail::SimdSizeType N, typename T, typename Abi>
	struct resize_simd<N, basic_simd<T, Abi>>
	{
		using type = basic_simd<T, detail::deduce_t<T, N>>;
	};
	template <detail::SimdSizeType N, std::size_t B, typename Abi>
	struct resize_simd<N, basic_simd_mask<B, Abi>>
	{
	...
	struct rebind_simd<U, basic_simd<T, Abi>>
	{
		using type = basic_simd<U, Abi>;
	};
	template <typename U, std::size_t B, typename Abi>
	struct rebind_simd<U, basic_simd_mask<B, Abi>>
	{
		using type = basic_simd_mask<sizeof(U), Abi>;
	};
	template <typename U, typename V>
	using rebind_simd_t = typename rebind_simd<U, V>::type;

	namespace detail
	{
		template <typename T>
		using is_vectorizable_t = is_vectorizable<T>;
	} // namespace detail
} // namespace ccm::pp
