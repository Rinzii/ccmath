...
namespace ccm::pp
{
	...
	public:
		...
		template <typename Flags = simd_flags<>> CCM_ALWAYS_INLINE void copy_from(T const * ptr, Flags /*flags*/ = {})
		{
			constexpr std::size_t req = detail::flags_align_request<Flags>::value;
			if constexpr (req != 0)
			{
				ptr = detail::assume_aligned_ptr<(req == 1 ? Traits::simd_align : req)>(ptr);
			}
			data_ = Traits::load(ptr);
		}
		template <typename Flags = simd_flags<>> CCM_ALWAYS_INLINE void copy_to(T * ptr, Flags /*flags*/ = {}) const
		{
			constexpr std::size_t req = detail::flags_align_request<Flags>::value;
			if constexpr (req != 0)
			{
		...
		CCM_ALWAYS_INLINE value_type operator[](detail::SimdSizeType i) const
		{
			return Traits::get(data_, i);
		}
		...
		CCM_ALWAYS_INLINE member_type & get()
		{
			return data_;
		}
		...
		{
			return from_member(Traits::add(a.data_, b.data_));
		}
		CCM_ALWAYS_INLINE friend basic_simd operator-(basic_simd const & a, basic_simd const & b)
		{
			return from_member(Traits::sub(a.data_, b.data_));
		}
		CCM_ALWAYS_INLINE friend basic_simd operator*(basic_simd const & a, basic_simd const & b)
		{
			return from_member(Traits::mul(a.data_, b.data_));
		}
		CCM_ALWAYS_INLINE friend basic_simd operator/(basic_simd const & a, basic_simd const & b)
		{
			return from_member(Traits::div(a.data_, b.data_));
		}
		CCM_ALWAYS_INLINE basic_simd operator-() const
		{
			return from_member(Traits::negate(data_));
		}
		CCM_ALWAYS_INLINE basic_simd operator+() const
		{
			return *this;
		...
		template <typename U = T, std::enable_if_t<std::is_integral_v<U>, int> = 0> CCM_ALWAYS_INLINE basic_simd operator~() const
		{
			return from_member(Traits::bnot(data_));
		}

		template <typename U = T, std::enable_if_t<std::is_integral_v<U>, int> = 0> CCM_ALWAYS_INLINE basic_simd & operator%=(basic_simd const & o)
		{
		...
		CCM_ALWAYS_INLINE friend mask_type operator==(basic_simd const & a, basic_simd const & b)
		{
			return mask_type::from_member(Traits::eq(a.data_, b.data_));
		}
		CCM_ALWAYS_INLINE friend mask_type operator!=(basic_simd const & a, basic_simd const & b)
		{
			return mask_type::from_member(Traits::ne(a.data_, b.data_));
		}
		CCM_ALWAYS_INLINE friend mask_type operator<(basic_simd const & a, basic_simd const & b)
		{
			return mask_type::from_member(Traits::lt(a.data_, b.data_));
		}
		CCM_ALWAYS_INLINE friend mask_type operator<=(basic_simd const & a, basic_simd const & b)
		{
			return mask_type::from_member(Traits::le(a.data_, b.data_));
		}
		CCM_ALWAYS_INLINE friend mask_type operator>(basic_simd const & a, basic_simd const & b)
		{
			return mask_type::from_member(Traits::gt(a.data_, b.data_));
		}
		CCM_ALWAYS_INLINE friend mask_type operator>=(basic_simd const & a, basic_simd const & b)
		{
			return mask_type::from_member(Traits::ge(a.data_, b.data_));
		}
	};
} // namespace ccm::pp
