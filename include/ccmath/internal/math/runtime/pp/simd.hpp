...
namespace ccm::pp
{
	...
	public:
		...
		template <typename U>
		explicit basic_simd(U value)->std::enable_if_t<std::is_convertible<U, T>::value, basic_simd> : data_(Traits::SimdBroadcast(value))
		{
		}

		// Load constructor
		template <typename Iter>
		basic_simd(Iter first)->std::enable_if_t<std::is_pointer<Iter>::value, basic_simd> : data_(Traits::SimdLoad(first, type_tag))
		{
		}
