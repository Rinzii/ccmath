...
namespace ccm::pp
{
	namespace detail
	{
		struct flag_aligned
		{};
		template <std::size_t N> struct flag_overaligned
		{};
	} // namespace detail
	...
	namespace detail
	{
		template <typename F> struct overalign_value : std::integral_constant<std::size_t, 0>
		{};
		template <std::size_t N> struct overalign_value<flag_overaligned<N>> : std::integral_constant<std::size_t, N>
		{};
		...
		template <typename Flags> struct flags_align_request : std::integral_constant<std::size_t, 0>
		{};
		// NOLINTBEGIN(readability-avoid-nested-conditional-operator)
		template <typename... Fs>
		struct flags_align_request<simd_flags<Fs...>>
			: std::integral_constant<std::size_t, ((std::size_t{0} + ... + overalign_value<Fs>::value) != 0)
		...
		template <std::size_t A, typename T> CCM_ALWAYS_INLINE T const * assume_aligned_ptr(T const * p)
		{
		...
		template <std::size_t A, typename T> CCM_ALWAYS_INLINE T * assume_aligned_ptr(T * p)
		{
#if defined(__clang__) || defined(__GNUC__)
	...
	{};
	template <typename... Flags> struct is_simd_flag_type<simd_flags<Flags...>> : std::true_type
	{};
	template <typename T> inline constexpr bool is_simd_flag_type_v = is_simd_flag_type<T>::value;

	// [simd.flags]
	inline constexpr simd_flags<> simd_flag_default{};
	inline constexpr simd_flags<detail::flag_aligned> simd_flag_aligned{};
	template <std::size_t N> inline constexpr simd_flags<detail::flag_overaligned<N>> simd_flag_overaligned{};
