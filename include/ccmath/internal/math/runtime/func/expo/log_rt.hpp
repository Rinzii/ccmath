...
namespace ccm::rt
{
	template <typename T, std::enable_if_t<std::is_floating_point_v<T>, bool> = true> [[nodiscard]] inline T log_rt(T num) noexcept
	{
		...
		return detail::sys::log_call(num);
#else
		if constexpr (ccm::builtin::has_runtime_log<T>)
		{
			...
			return ccm::builtin::log_rt(num);
		} else
		{
	#if defined(CCMATH_HAS_SIMD) && defined(CCMATH_HAS_SIMD_SVML) && !defined(_MSC_VER)
			return detail::unary_svml_or_impl(num, [](auto v) { return intrin::log(v); }, scalar);
	#else
