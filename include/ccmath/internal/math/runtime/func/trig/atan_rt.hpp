...
namespace ccm::rt
{
	template <typename T, std::enable_if_t<std::is_floating_point_v<T>, bool> = true> [[nodiscard]] inline T atan_rt(T num) noexcept
	{
		if constexpr (ccm::builtin::has_runtime_atan<T>)
		{
			return ccm::builtin::atan_rt(num);
		} else
		{
