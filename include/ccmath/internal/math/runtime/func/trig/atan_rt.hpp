...
namespace ccm::rt
{
	...
	[[nodiscard]] inline T atan_rt(T num) noexcept
	{
		...
		else { return static_cast<T>(__builtin_atanl(static_cast<long double>(num))); }
#else
		const auto scalar = [](T value) {
			return detail::dispatch_float_double(value, internal::impl::atan_float, internal::impl::atan_double);
		};
