...
namespace ccm::rt::simd_impl
{
	...
	[[nodiscard]] inline T pow_simd_impl(T base, T exp) noexcept
	{
		...
		{
			return gen::impl::powf_simd(base_v, exp_v)[0];
		} else if constexpr (std::is_same_v<T, double>)
		{
			return gen::impl::pow_simd(base_v, exp_v)[0];
		} else
		{
...
namespace ccm::rt
{
	template <typename T, std::enable_if_t<std::is_floating_point_v<T>, bool> = true> T pow_rt(T base, T exp)
	{
		...
		{
			return simd_impl::pow_simd_impl(base, exp);
		} else
		{
			return gen::pow_gen<T>(base, exp);
		}
