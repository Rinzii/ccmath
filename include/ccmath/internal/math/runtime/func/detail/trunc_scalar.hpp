...
{
	template <typename T>
	[[nodiscard]] inline T trunc_scalar(T num) noexcept
	{
		...
		if (CCM_UNLIKELY(num == static_cast<T>(0))) { return num; }
	...
	template <typename T>
	[[nodiscard]] inline T floor_scalar(T num) noexcept
	{
	...
	template <typename T>
	[[nodiscard]] inline T ceil_scalar(T num) noexcept
	{
		if (ccm::isinf(num) || num == static_cast<T>(0) || ccm::isnan(num)) { return num; }
