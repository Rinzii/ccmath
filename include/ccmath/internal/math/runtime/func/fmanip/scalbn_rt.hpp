...
namespace ccm::rt
{
	template <typename T, std::enable_if_t<std::is_floating_point_v<T>, bool> = true> [[nodiscard]] inline T scalbn_rt(T num, int exp) noexcept
	{
		if constexpr (ccm::builtin::has_runtime_scalbn<T>)
		{
			return ccm::builtin::scalbn_rt(num, exp);
		} else
		{
