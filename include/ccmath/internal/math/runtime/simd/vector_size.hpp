...
namespace ccm::intrin
{
	namespace abi
	{
		...
		template <int N> struct vector_size
		{};

	} // namespace abi

	template <class T, int N> struct simd_mask<T, abi::vector_size<N>>
	{
	private:
		using native_type =
	...
	public:
		...
		{
			return reinterpret_cast<int *>(&m_value)[i];
		} // NOLINT(cppcoreguidelines-pro-type-reinterpret-cast)
		[[nodiscard]] CCM_ALWAYS_INLINE native_type const & get() const
		{
			return m_value;
		}
	...
	template <int N> struct simd_mask<double, abi::vector_size<N>>
	{
	private:
	...
	public:
		...
		{
			return reinterpret_cast<long long *>(&m_value)[i];
		} // NOLINT(cppcoreguidelines-pro-type-reinterpret-cast)
		[[nodiscard]] CCM_ALWAYS_INLINE native_type const & get() const
		{
			return m_value;
	...
	template <class T, int N> CCM_ALWAYS_INLINE CCM_GPU_HOST_DEVICE bool all_of(simd_mask<T, abi::vector_size<N>> const & a)
	{
		bool result = true;
	...
	template <class T, int N> CCM_ALWAYS_INLINE CCM_GPU_HOST_DEVICE bool any_of(simd_mask<T, abi::vector_size<N>> const & a)
	{
		bool result = false;
		CCM_SIMD_VECTORIZE for (int i = 0; i < a.size(); ++i)
		{
	...
	template <class T, int N> struct simd<T, abi::vector_size<N>>
	{
	private:
	...
	public:
		...
		CCM_ALWAYS_INLINE explicit simd(T value)
		{
			...
			{
				reinterpret_cast<T *>(&m_value)[i] = value;
			} // NOLINT(cppcoreguidelines-pro-type-reinterpret-cast)
		}
		explicit CCM_ALWAYS_INLINE simd(const native_type & value) : m_value(value) {}
		CCM_ALWAYS_INLINE explicit simd(storage_type const & value)
		{
		...
		CCM_ALWAYS_INLINE native_type & get()
		{
			return m_value;
		}
