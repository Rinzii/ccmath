...
namespace ccm::intrin
{
	...
	struct simd_mask<float, abi::avx>
	{
		...
		CCM_ALWAYS_INLINE
		simd_mask(bool value) : m_value(_mm256_castsi256_ps(_mm256_set1_epi32(-static_cast<int>(value)))) {} // NOLINT
	...
	struct simd<float, abi::avx>
	{
		...
		static constexpr int size() { return 8; }
		CCM_ALWAYS_INLINE
		simd(float value) : m_value(_mm256_set1_ps(value)) {} // NOLINT
		CCM_ALWAYS_INLINE
		simd(float a, float b, float c, float d, float e, float f, float g, float h) : m_value(_mm256_setr_ps(a, b, c, d, e, f, g, h)) {}
		CCM_ALWAYS_INLINE
		simd(storage_type const & value) { copy_from(value.data(), element_aligned_tag()); } // NOLINT
		...
		CCM_ALWAYS_INLINE
		simd(float const * ptr, int stride)
		...
		CCM_ALWAYS_INLINE simd_mask<float, abi::avx> operator<(simd const & other) const { return {_mm256_cmp_ps(m_value, other.m_value, _CMP_LT_OS)}; }
		CCM_ALWAYS_INLINE simd_mask<float, abi::avx> operator==(simd const & other) const { return {_mm256_cmp_ps(m_value, other.m_value, _CMP_EQ_OS)}; }

	private:
		__m256 m_value;
	};

	CCM_ALWAYS_INLINE simd<float, abi::avx> choose(simd_mask<float, abi::avx> const & a, simd<float, abi::avx> const & b, simd<float, abi::avx> const & c)
	{
	...
	struct simd_mask<double, abi::avx>
	{
		...
		CCM_ALWAYS_INLINE simd_mask() = default;
		CCM_ALWAYS_INLINE
		simd_mask(bool value) : m_value(_mm256_castsi256_pd(_mm256_set1_epi64x(-static_cast<std::int64_t>(value)))) {} // NOLINT
	...
	public:
		...
		CCM_ALWAYS_INLINE static constexpr int size() { return 4; }
		CCM_ALWAYS_INLINE
		simd(double value) : m_value(_mm256_set1_pd(value)) {} // NOLINT
		CCM_ALWAYS_INLINE
		...
		CCM_ALWAYS_INLINE
		simd(double const * ptr, int stride) : simd(ptr[0], ptr[stride], ptr[2 * stride], ptr[3 * stride]) {} // NOLINT
		...
		CCM_ALWAYS_INLINE simd_mask<double, abi::avx> operator<(simd const & other) const { return {_mm256_cmp_pd(m_value, other.m_value, _CMP_LT_OS)}; }
		CCM_ALWAYS_INLINE simd_mask<double, abi::avx> operator==(simd const & other) const { return {_mm256_cmp_pd(m_value, other.m_value, _CMP_EQ_OS)}; }

	private:
		__m256d m_value;
	};

	CCM_ALWAYS_INLINE simd<double, abi::avx> choose(simd_mask<double, abi::avx> const & a, simd<double, abi::avx> const & b, simd<double, abi::avx> const & c)
	{
