...
namespace ccm::intrin
{
	...
	template <> struct simd_mask<float, abi::neon>
	{
		...
		[[nodiscard]] CCM_ALWAYS_INLINE constexpr uint32x4_t get() const
		{
			return m_value;
		}
	...
	template <> struct simd<float, abi::neon>
	{
		...
		template <class Flags> CCM_ALWAYS_INLINE simd(float const * ptr, Flags flags)
		{
			copy_from(ptr, flags);
		}
		...
		CCM_ALWAYS_INLINE void copy_from(float const * ptr, element_aligned_tag)
		{
			m_value = vld1q_f32(ptr);
		}
		CCM_ALWAYS_INLINE void copy_to(float * ptr, element_aligned_tag) const
		{
			vst1q_f32(ptr, m_value);
		}
		[[nodiscard]] CCM_ALWAYS_INLINE constexpr float32x4_t get() const
		{
			return m_value;
		}
	...
	template <> struct simd_mask<double, abi::neon>
	{
		using value_type			  = bool;
		...
		[[nodiscard]] CCM_ALWAYS_INLINE constexpr uint64x2_t get() const
		{
			return m_value;
		}
		CCM_ALWAYS_INLINE simd_mask operator||(simd_mask const & other) const
	...
	template <> struct simd<double, abi::neon>
	{
		...
		template <class Flags> CCM_ALWAYS_INLINE simd(double const * ptr, Flags flags)
		{
			copy_from(ptr, flags);
		}
		...
		CCM_ALWAYS_INLINE void copy_from(double const * ptr, element_aligned_tag)
		{
			m_value = vld1q_f64(ptr);
		}
		CCM_ALWAYS_INLINE void copy_to(double * ptr, element_aligned_tag) const
		{
			vst1q_f64(ptr, m_value);
		}
		[[nodiscard]] CCM_ALWAYS_INLINE constexpr float64x2_t get() const
		{
			return m_value;
		}
