...
namespace ccm::intrin
{
	...
	struct simd<float, abi::ssse3>
	{
		...
		static constexpr int size() { return 4; }
		CCM_ALWAYS_INLINE
		simd(float value) : m_value(_mm_set1_ps(value)) {} // NOLINT
		CCM_ALWAYS_INLINE
		...
		CCM_ALWAYS_INLINE
		simd(float const * ptr, int stride) : simd(ptr[0], ptr[stride], ptr[2 * stride], ptr[3 * stride]) {} // NOLINT
		CCM_ALWAYS_INLINE constexpr simd(__m128 const & value_in) : m_value(value_in) {}					 // NOLINT
	...
	CCM_ALWAYS_INLINE simd<float, abi::ssse3>
	choose(simd_mask<float, abi::ssse3> const & a, simd<float, abi::ssse3> const & b, simd<float, abi::ssse3> const & c)
	{
	...
	template <>
	struct simd<double, abi::ssse3> // NOLINT
	{
		...
		CCM_ALWAYS_INLINE
		simd(double value) : m_value(_mm_set1_pd(value)) {} // NOLINT
		CCM_ALWAYS_INLINE
		...
		CCM_ALWAYS_INLINE
		simd(double const * ptr, int stride) : simd(ptr[0], ptr[stride]) {}				  // NOLINT
		CCM_ALWAYS_INLINE constexpr simd(__m128d const & value_in) : m_value(value_in) {} // NOLINT
	...
	CCM_ALWAYS_INLINE simd<double, abi::ssse3>
	choose(simd_mask<double, abi::ssse3> const & a, simd<double, abi::ssse3> const & b, simd<double, abi::ssse3> const & c)
	{
		// NOLINTNEXTLINE(modernize-return-braced-init-list)
