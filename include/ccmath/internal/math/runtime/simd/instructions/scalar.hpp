...
namespace ccm::intrin
{
	...
	template <class T> struct simd_mask<T, abi::scalar>
	{
		using value_type			  = bool;
		using simd_type				  = simd<T, abi::scalar>;
		using abi_type				  = abi::scalar;
		CCM_ALWAYS_INLINE simd_mask() = default;
		CCM_ALWAYS_INLINE CCM_GPU_HOST_DEVICE static constexpr int size()
		{
			return 1;
		}
		CCM_ALWAYS_INLINE CCM_GPU_HOST_DEVICE explicit simd_mask(bool value) : m_value(value) {}
		[[nodiscard]] CCM_GPU_HOST_DEVICE constexpr bool get() const
		{
			return m_value;
		}
		CCM_ALWAYS_INLINE CCM_GPU_HOST_DEVICE simd_mask operator||(simd_mask const & other) const
		{
		...
		CCM_ALWAYS_INLINE CCM_GPU_HOST_DEVICE simd_mask operator!() const
		{
			return !m_value;
		}
	...
	template <class T> struct simd_storage<T, abi::scalar>
	{
	private:
	...
	public:
		...
		CCM_ALWAYS_INLINE CCM_GPU_HOST_DEVICE T const * data() const
		{
			return &m_value;
		}
		CCM_ALWAYS_INLINE CCM_GPU_HOST_DEVICE T * data()
		{
			return &m_value;
		}
		CCM_ALWAYS_INLINE CCM_GPU_HOST_DEVICE T const & operator[](int /*unused*/) const
		{
			return m_value;
		}
		CCM_ALWAYS_INLINE CCM_GPU_HOST_DEVICE T & operator[](int /*unused*/)
		{
			return m_value;
		}
	...
	template <class T> CCM_ALWAYS_INLINE CCM_GPU_HOST_DEVICE bool all_of(simd_mask<T, abi::scalar> const & a)
	{
		return a.get();
	}

	template <class T> CCM_ALWAYS_INLINE CCM_GPU_HOST_DEVICE bool any_of(simd_mask<T, abi::scalar> const & a)
	{
		return a.get();
	}
	...
	struct simd<T, abi::scalar>
	{
		...
		template <class Flags> CCM_ALWAYS_INLINE CCM_GPU_HOST_DEVICE simd(T const * ptr, Flags flags)
		{
			copy_from(ptr, flags);
		}
		...
		CCM_ALWAYS_INLINE CCM_GPU_HOST_DEVICE simd operator*(simd const & other) const
		{
		...
		CCM_ALWAYS_INLINE CCM_GPU_HOST_DEVICE void copy_from(T const * ptr, element_aligned_tag /*unused*/)
		{
			m_value = *ptr;
		}
		CCM_ALWAYS_INLINE CCM_GPU_HOST_DEVICE void copy_to(T * ptr, element_aligned_tag /*unused*/) const
		{
			*ptr = m_value;
		}
		CCM_ALWAYS_INLINE CCM_GPU_HOST_DEVICE constexpr T get() const
		{
			return m_value;
		}
		CCM_ALWAYS_INLINE CCM_GPU_HOST_DEVICE T convert() const
		{
			return m_value;
		}
