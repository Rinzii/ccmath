...
namespace ccm::intrin
{
	...
	template <> struct simd_mask<float, abi::sse4>
	{
		using value_type			  = bool;
		using simd_type				  = simd<float, abi::sse4>;
		using abi_type				  = abi::sse4;
		...
		[[nodiscard]] CCM_ALWAYS_INLINE constexpr __m128 get() const
		{
			return m_value;
		}
	...
	CCM_ALWAYS_INLINE bool all_of(simd_mask<float, abi::sse4> const & a)
	{
		return _mm_movemask_ps(a.get()) == 0xF;
	}

	CCM_ALWAYS_INLINE bool any_of(simd_mask<float, abi::sse4> const & a)
	{
		return _mm_movemask_ps(a.get()) != 0x0;
	}

	template <> struct simd<float, abi::sse4>
	{
		...
		template <class Flags> CCM_ALWAYS_INLINE simd(float const * ptr, Flags /*flags*/) : m_value(_mm_loadu_ps(ptr)) {}
		...
		CCM_ALWAYS_INLINE void copy_to(float * ptr, element_aligned_tag /*unused*/) const
		{
		...
		[[nodiscard]] CCM_ALWAYS_INLINE float convert() const
		{
			return _mm_cvtss_f32(m_value);
		}
		CCM_ALWAYS_INLINE simd_mask<float, abi::sse4> operator<(simd const & other) const
		{
			return simd_mask<float, abi::sse4>(_mm_cmplt_ps(m_value, other.m_value));
		}
		CCM_ALWAYS_INLINE simd_mask<float, abi::sse4> operator==(simd const & other) const
		{
			return simd_mask<float, abi::sse4>(_mm_cmpeq_ps(m_value, other.m_value));
		}
	...
	CCM_ALWAYS_INLINE simd<float, abi::sse4> choose(simd_mask<float, abi::sse4> const & a, simd<float, abi::sse4> const & b, simd<float, abi::sse4> const & c)
	{
		// NOLINTNEXTLINE(modernize-return-braced-init-list)
		return simd<float, abi::sse4>(_mm_add_ps(_mm_and_ps(a.get(), b.get()), _mm_andnot_ps(a.get(), c.get())));
	}

	template <> struct simd_mask<double, abi::sse4>
	{
		using value_type			  = bool;
		using simd_type				  = simd<double, abi::sse4>;
		using abi_type				  = abi::sse4;
		CCM_ALWAYS_INLINE simd_mask() = default;
		...
		[[nodiscard]] constexpr __m128d get() const
		{
			return m_value;
		}
		CCM_ALWAYS_INLINE simd_mask operator||(simd_mask const & other) const
	...
	CCM_ALWAYS_INLINE bool all_of(simd_mask<double, abi::sse4> const & a)
	{
		return _mm_movemask_pd(a.get()) == 0x3;
	}

	CCM_ALWAYS_INLINE bool any_of(simd_mask<double, abi::sse4> const & a)
	{
		return _mm_movemask_pd(a.get()) != 0x0;
	}

	template <> struct simd<double, abi::sse4>
	{
		...
		template <class Flags> CCM_ALWAYS_INLINE simd(double const * ptr, Flags /*flags*/) : m_value(_mm_loadu_pd(ptr)) {}
		...
		CCM_ALWAYS_INLINE void copy_to(double * ptr, element_aligned_tag /*unused*/) const
		{
		...
		[[nodiscard]] CCM_ALWAYS_INLINE double convert() const
		{
			return _mm_cvtsd_f64(m_value);
		}

		CCM_ALWAYS_INLINE simd_mask<double, abi::sse4> operator<(simd const & other) const
		{
			return simd_mask<double, abi::sse4>(_mm_cmplt_pd(m_value, other.m_value));
		}
		CCM_ALWAYS_INLINE simd_mask<double, abi::sse4> operator==(simd const & other) const
		{
			return simd_mask<double, abi::sse4>(_mm_cmpeq_pd(m_value, other.m_value));
		}
