...
namespace ccm::support::fp
{
	...
	namespace internal
	{
		...
		template <> struct FPLayout<FPType::eBinary32>
		{
		...
		template <> struct FPLayout<FPType::eBinary64>
		{
		...
		template <FPType fp_type> struct FPStorage : FPLayout<fp_type>
		{
		...
		protected:
			...
			template <typename T> struct TypedInt // NOLINT(cppcoreguidelines-special-member-functions)
			{
				...
				[[nodiscard]] constexpr storage_type to_storage_type() const
				{
					return storage_type(value);
				}

				friend constexpr bool operator==(TypedInt a, TypedInt b)
				{
			...
			struct Exponent : TypedInt<std::int32_t>
			{
				...
				{
					return Exponent(1 - exponent_bias);
				}
				...
				{
					return Exponent(exponent_bias + 1);
				}
			};
			...
			struct BiasedExponent : TypedInt<std::uint32_t>
			{
				...
				{
					return Exponent(static_cast<std::int32_t>(BASE::value) - exponent_bias);
				}
			...
			struct Significand : TypedInt<storage_type>
			{
				...
				friend constexpr Significand operator>>(const Significand a, int shift)
				{
					return Significand(storage_type(a.to_storage_type() >> shift));
				}

				static constexpr auto zero()
				{
					return Significand(storage_type(0));
				}

				static constexpr auto lsb()
				{
					return Significand(storage_type(1));
				}

				static constexpr auto msb()
				{
					return Significand(storage_type(1) << (significand_length - 1));
				}

				static constexpr auto bits_all_ones()
				{
					return Significand(significand_mask);
				}
			};

			static constexpr storage_type encode(BiasedExponent exp)
			{
				return (exp.to_storage_type() << significand_length) & exponent_mask;
			}

			static constexpr storage_type encode(Significand value)
			{
				return value.to_storage_type() & significand_mask;
			}

			static constexpr storage_type encode(BiasedExponent exp, Significand sig)
			{
			...
			[[nodiscard]] constexpr storage_type exp_bits() const
			{
				return bits & exponent_mask;
			}

			[[nodiscard]] constexpr storage_type sig_bits() const
			{
				return bits & significand_mask;
			}

			[[nodiscard]] constexpr storage_type exp_sig_bits() const
			{
				return bits & exponent_significand_mask;
			}
			...
			{
				bits = merge(bits, encode(biased), exponent_mask);
			}
		...
		public:
			[[nodiscard]] constexpr types::Sign sign() const
			{
				return (bits & sign_mask) ? types::Sign::NEG : types::Sign::POS;
			}
		 ...
		 */
		template <FPType fp_type, typename RetT> struct FPRepSem : FPStorage<fp_type>
		{
		...
		public:
			...
			static constexpr RetT zero(types::Sign sign = types::Sign::POS)
			{
				return RetT(encode(sign, Exponent::subnormal(), Significand::zero()));
			}

			static constexpr RetT one(types::Sign sign = types::Sign::POS)
			{
				return RetT(encode(sign, Exponent::zero(), Significand::zero()));
			}

			static constexpr RetT min_subnormal(types::Sign sign = types::Sign::POS)
			{
			...
			static constexpr RetT min_normal(types::Sign sign = types::Sign::POS)
			{
				return RetT(encode(sign, Exponent::min(), Significand::zero()));
			}

			static constexpr RetT max_normal(types::Sign sign = types::Sign::POS)
			{
				return RetT(encode(sign, Exponent::max(), Significand::bits_all_ones()));
			}

			static constexpr RetT inf(types::Sign sign = types::Sign::POS)
			{
			...
			constexpr RetT next_toward_inf() const
			{
				if (is_finite())
				{
					return RetT(bits + storage_type(1));
				}
				return RetT(bits);
			}
		...
		public:
			...
			constexpr RetT next_toward_inf() const
			{
				if (is_finite())
				{
					...
					if (exp_sig_bits() == max_subnormal().uintval())
					{
						return min_normal(sign());
					}
					if (sig_bits() == significand_mask)
					{
			...
			[[nodiscard]] constexpr storage_type get_explicit_mantissa() const
			{
				return sig_bits();
			}
		 ...
		 */
		template <FPType fp_type, typename RetT> struct FPRepImpl : FPRepSem<fp_type, RetT>
		{
		...
		public:
			...
			constexpr friend bool operator==(FPRepImpl a, FPRepImpl b)
			{
			...
			// Representation
			constexpr storage_type uintval() const
			{
				return bits & fp_bits_mask;
			}
			constexpr void set_uintval(storage_type value)
			{
				bits = (value & fp_bits_mask);
			}
			...
			constexpr RetT abs() const
			{
				return RetT(bits & BASE::exponent_significand_mask);
			}
			...
			[[nodiscard]] constexpr bool is_inf_or_nan() const
			{
				return !is_finite();
			}
			...
			[[nodiscard]] constexpr bool is_pos() const
			{
				return sign().is_pos();
			}
			...
			[[nodiscard]] constexpr int get_exponent() const
			{
				return static_cast<std::int32_t>(Exponent(BASE::biased_exponent()));
			}
			...
			constexpr storage_type get_mantissa() const
			{
			...
			{
				bits = BASE::merge(bits, mantVal, fraction_mask);
			}
			...
			{
				bits = BASE::merge(bits, sigVal, significand_mask);
			}
		...
		struct FPRep : FPRepImpl<fp_type, FPRep<fp_type>>
		{
			...
			constexpr explicit operator storage_type() const
			{
				return BASE::uintval();
			}
		};
	...
	template <typename T> struct FPBits final : internal::FPRepImpl<get_fp_type<T>(), FPBits<T>>
	{
		...
		template <typename XType> constexpr explicit FPBits(XType x)
		{
			...
			{
				BASE::bits = support::bit_cast<storage_type>(x);
			} else if constexpr (std::is_same_v<UnQual, storage_type>)
			{
				BASE::bits = x;
			} else
			{
		...
		{
			return support::bit_cast<T>(BASE::bits);
		}
	};
} // namespace ccm::support::fp
