...
namespace ccm::support
{
	...
	std::enable_if_t<sizeof(To) == sizeof(From), To> constexpr bit_cast(const From & from) noexcept
	{
		static_assert(std::is_trivially_copyable_v<From> && std::is_trivially_copyable_v<To>,
					  "This implementation additionally requires both source and destination types to be trivially copyable");
