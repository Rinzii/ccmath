...
namespace ccm::intrin
{
	...
	CCM_ALWAYS_INLINE inline bool all_of(simd_mask<float, abi::ssse3> const & a)
	{
		return _mm_movemask_ps(a.get()) == 0xF;
	}

	CCM_ALWAYS_INLINE inline bool any_of(simd_mask<float, abi::ssse3> const & a)
	{
	...
	CCM_ALWAYS_INLINE inline bool all_of(simd_mask<double, abi::ssse3> const & a)
	{
		return _mm_movemask_pd(a.get()) == 0x3;
	}

	CCM_ALWAYS_INLINE inline bool any_of(simd_mask<double, abi::ssse3> const & a)
	{
	...
	struct simd<double, abi::ssse3>
	{
		...
		CCM_ALWAYS_INLINE inline simd_mask<double, abi::ssse3> operator<(simd const & other) const
		{
