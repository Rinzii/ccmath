...
namespace ccm::gen
{
	namespace internal::impl
	{
		...
		constexpr auto convert_fp_to_uint(T x) noexcept
		{
			...
			const Storage_t x_abs = x_bits.abs().uintval();
			return x_abs;
		}
		...
		constexpr bool is_odd_integer(T x)
		{
			...
			const auto x_u				 = support::bit_cast<Storage_t>(x);
			const auto x_e				 = static_cast<SignedStorage_t>((x_u & FPBits_t::exponent_mask) >> FPBits_t::fraction_length);
			const auto lsb				 = support::countr_zero(x_u | FPBits_t::exponent_mask);
		...
		constexpr bool is_integer(T x)
		{
			...
			using Storage_t							= typename FPBits_t::storage_type;
			using SignedStorage_t					= support::float_signed_bits_t<T>;
			const Storage_t x_u						= support::bit_cast<Storage_t>(x);
			const auto x_e							= static_cast<SignedStorage_t>((x_u & FPBits_t::exponent_mask) >> FPBits_t::fraction_length);
			const SignedStorage_t lsb				= support::countr_zero(x_u | FPBits_t::exponent_mask);
		...
		constexpr std::enable_if_t<std::is_floating_point_v<T>, T> pow_impl_handle_special_cases(support::fp::FPBits<T> & x_bits,
																								 support::fp::FPBits<T> & y_bits, T & x, T & y) noexcept
		{
			if (x_bits.is_nan())
			{
	...
	(defined(__clang__) && defined(CCM_CONFIG_AGGRESSIVELY_OPTIMIZE)) || defined(__INTEL_LLVM_COMPILER)
		...
		template <typename T>
		constexpr std::enable_if_t<std::is_floating_point_v<T>, T> pow_impl_old(T x, T y) noexcept
		{
	...
	template <typename T>
	constexpr std::enable_if_t<std::is_floating_point_v<T>, T> pow_gen_old(T base, T exp) noexcept
	{
		return internal::impl::pow_impl_old(base, exp);
	}
} // namespace ccm::gen