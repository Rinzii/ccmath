	...
	namespace multiword
	{
		...
		constexpr DoubleWide<word> mul2(word lhs, word rhs)
		{
			...
			else
			{
				...
				const word step1	= rhs_lo * lhs_lo;
				const word step2	= rhs_lo * lhs_hi;
				const word step3	= rhs_hi * lhs_lo;
				const word step4	= rhs_hi * lhs_hi;
				word lo_digit		= step1;
				word hi_digit		= step4;
...
#define DECLARE_COUNTBIT(NAME, INDEX_EXPR)                                                                                                                     \
	...
	public:
		...
		*/
		...
		{
			return std::all_of(val.begin(), val.end(), [](auto part) { return part == 0; });
		}
		...
		{
			BigInt result = *this;
			result.sub_overflow(std::move(other));
			return result;
		}
...
namespace ccm::support
{
	...
	constexpr std::enable_if_t<sizeof(To) == sizeof(ccm::types::UInt<Bits>) && std::is_trivially_constructible_v<To> && std::is_trivially_copyable_v<To> &&
								   std::is_trivially_copyable_v<ccm::types::UInt<Bits>>,
							   To>
