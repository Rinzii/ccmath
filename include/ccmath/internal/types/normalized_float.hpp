...
namespace ccm::types
{
	...
	template <typename T> struct NormalizedFloat
	{
		...
		constexpr explicit NormalizedFloat(T x)
		{
			init_from_bits(FPBits(x));
		}

		constexpr explicit NormalizedFloat(FPBits bits)
		{
			init_from_bits(bits);
		}
	...
	template <typename T> using NormalFloat = NormalizedFloat<T>;
