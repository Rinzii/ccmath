...
{
	for ([[maybe_unused]] auto _ : state) { benchmark::DoNotOptimize(std::abs(state.range(0))); }
	state.SetComplexityN(state.range(0));
}

static void BM_basic_abs_ccmath(benchmark::State& state)
{
	for ([[maybe_unused]] auto _ : state) { benchmark::DoNotOptimize(ccm::abs(state.range(0))); }
	state.SetComplexityN(state.range(0));
}
...
static void BM_basic_abs_rand_double_std(benchmark::State& state)
{
	...
	while (state.KeepRunning())
	{
		for (auto x : randomDoubles) { benchmark::DoNotOptimize(std::abs(x)); }
	}
	state.SetComplexityN(state.range(0));
}
...
static void BM_basic_abs_rand_double_ccmath(benchmark::State& state)
{
	...
	while (state.KeepRunning())
	{
		for (auto x : randomDoubles) { benchmark::DoNotOptimize(ccm::abs(x)); }
	}
	state.SetComplexityN(state.range(0));
}
