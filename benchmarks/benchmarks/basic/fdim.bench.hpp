...
{
	for ([[maybe_unused]] auto _ : state) { benchmark::DoNotOptimize(std::fdim(state.range(0), state.range(1))); }
	state.SetComplexityN(state.range(0));
}

static void BM_basic_fdim_ccmath(benchmark::State& state)
{
	for ([[maybe_unused]] auto _ : state) { benchmark::DoNotOptimize(ccm::fdim(state.range(0), state.range(1))); }
	state.SetComplexityN(state.range(0));
}
...
static void BM_basic_fdim_rand_int_std(benchmark::State& state)
{
	...
	{
		for (size_t i = 0; i < randomIntegers.size(); ++i) { benchmark::DoNotOptimize(std::fdim(randomIntegers[i], randomIntegers2[i])); }
	}
	state.SetComplexityN(state.range(0));
}
...
static void BM_basic_fdim_rand_int_ccmath(benchmark::State& state)
{
	...
	while (state.KeepRunning())
	{
		for (size_t i = 0; i < randomIntegers.size(); ++i) { benchmark::DoNotOptimize(ccm::fdim(randomIntegers[i], randomIntegers2[i])); }
	}
	state.SetComplexityN(state.range(0));
}
...
static void BM_basic_fdim_rand_double_std(benchmark::State& state)
{
	...
	while (state.KeepRunning())
	{
		for (size_t i = 0; i < randomDoubles.size(); ++i) { benchmark::DoNotOptimize(std::fdim(randomDoubles[i], randomDoubles2[i])); }
	}
	state.SetComplexityN(state.range(0));
}
...
static void BM_basic_fdim_rand_double_ccmath(benchmark::State& state)
{
	...
	while (state.KeepRunning())
	{
		for (size_t i = 0; i < randomDoubles.size(); ++i) { benchmark::DoNotOptimize(ccm::fdim(randomDoubles[i], randomDoubles2[i])); }
	}
	state.SetComplexityN(state.range(0));
}
