...
{
	for ([[maybe_unused]] auto _ : state) { benchmark::DoNotOptimize(std::fma(state.range(0), state.range(1), state.range(2))); }
	state.SetComplexityN(state.range(0));
}

static void BM_basic_fma_ccmath(benchmark::State& state)
{
	for ([[maybe_unused]] auto _ : state) { benchmark::DoNotOptimize(ccm::fma(state.range(0), state.range(1), state.range(2))); }
	state.SetComplexityN(state.range(0));
}
...
static void BM_basic_fma_rand_int_std(benchmark::State& state)
{
	...
	{
		for (size_t i = 0; i < randomIntegers.size(); ++i) { benchmark::DoNotOptimize(std::fma(randomIntegers[i], randomIntegers2[i], randomIntegers3[i])); }
	}
	state.SetComplexityN(state.range(0));
}
...
static void BM_basic_fma_rand_int_ccmath(benchmark::State& state)
{
	...
	while (state.KeepRunning())
	{
		for (size_t i = 0; i < randomIntegers.size(); ++i) { benchmark::DoNotOptimize(ccm::fma(randomIntegers[i], randomIntegers2[i], randomIntegers3[i])); }
	}
	state.SetComplexityN(state.range(0));
}
...
static void BM_basic_fma_rand_double_std(benchmark::State& state)
{
	...
	while (state.KeepRunning())
	{
		for (size_t i = 0; i < randomDoubles.size(); ++i) { benchmark::DoNotOptimize(std::fma(randomDoubles[i], randomDoubles2[i], randomDoubles3[i])); }
	}
	state.SetComplexityN(state.range(0));
}
...
static void BM_basic_fma_rand_double_ccmath(benchmark::State& state)
{
	...
	while (state.KeepRunning())
	{
		for (size_t i = 0; i < randomDoubles.size(); ++i) { benchmark::DoNotOptimize(ccm::fma(randomDoubles[i], randomDoubles2[i], randomDoubles3[i])); }
	}
	state.SetComplexityN(state.range(0));
}
