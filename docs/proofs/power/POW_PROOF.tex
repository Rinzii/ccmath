...
renormalization.  Two guards make the algebraic path safe to trust.  First, the
positive power $x^{|y|}$, held as the pair $p$, must satisfy
$2^{-960} \le |p_{\mathrm{hi}}| \le 2^{960}$; outside that band the double-double
low limb underflows or overflows during squaring and the pair loses the extra
precision the path exists to provide (this bites a tiny base with a large
negative exponent, whose reciprocal is a large normal result), so such inputs
fall through to the kernel.  Second, a negative result's sign is folded into the
pair before the final rounding rather than negated afterward: negating a
magnitude that was already rounded in the active mode flips directed rounding,
since $-\,\mathrm{RU}(|v|) = \mathrm{RD}(-|v|)$.  For negative $y$ one Newton
reciprocal of the (signed) pair is
...
the odd integer $2y$ by the same square-and-multiply, subject to the same
safe-band guard. If the algebraic result is
