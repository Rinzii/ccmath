...
integer results carry a sub-ULP tail that decides the directed rounding).  The
binary weight is reattached at the end by a single power-of-two scaling, exact
whenever the final value is normal.  Two further points make the path safe.
First, a power whose exponent estimate $|y| \cdot (e_x + 1)$ exceeds $1023$ is
screened out before the squaring, because an overflowing product clamps to a
finite maximal value under directed rounding rather than becoming infinity and
would otherwise be mistaken for a valid result.  Second, a negative result's
sign is folded into the pair before the final rounding rather than negated
afterward: negating a magnitude that was already rounded in the active mode flips
directed rounding, since $-\,\mathrm{RU}(|v|) = \mathrm{RD}(-|v|)$.  For negative
$y$ one Newton reciprocal of the (signed) mantissa pair is taken before the
weight is reattached.  For half-integer $y$ with $|2y| \le 2048$ the identity
...
the odd integer $2y$ by double-double square-and-multiply, screened by the same
overflow estimate and a low-limb magnitude check.  If the algebraic result is
