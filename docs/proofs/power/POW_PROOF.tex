...
returns one binary64 rounding of $r_{\mathrm{hi}} + r_{\mathrm{lo}}$ for a normal
final value.  The
...
$2^{\cdot/64}$, and collapses the final pair with one binary64 addition for a
normal result.  The
two minimax polynomials are the new numeric artifacts; their approximation
error is certified below.

Two reconstruction details are required for correct rounding in every mode and
are worth recording, since neither is captured by the polynomial certificates.
First, the high term $e_x + \log_2(r_1)$ is formed as an \texttt{exact\_add} so
that its rounding residual survives in the low limb: for a base far from $1$ the
integer $e_x$ is large and a bare single-double sum drops up to half an ulp of
$e_x$, which the subsequent multiplication by $y$ amplifies past a full result
ulp.  Second, when the over/underflow scale $2^{-512}$ is in force and the final
value is subnormal, collapsing the pair to a normal double and then applying the
scale rounds twice; a round-to-odd intermediate cannot repair this, because a
$53$-bit double leaves only one guard bit over a $52$-bit subnormal while
round-to-odd needs two.  The kernel therefore rounds the double-double directly
onto the shared subnormal grid (lsb $2^{-1074}$) in a single active-mode
rounding, after which the power-of-two reattachment is exact.
...
renormalization.  The squaring carries a separate binary exponent: after each
product the mantissa pair is rescaled so its high limb lies in $[1,2)$ and the
shift is accumulated, so neither limb leaves the normal range however small or
large $x^{|y|}$ becomes (a naive accumulation loses the low limb once an
intermediate nears the subnormal floor, for instance a small base raised to a
large power or its reciprocal, which is exactly where these near-power-of-two
integer results carry a sub-ULP tail that decides the directed rounding).  A
negative result's sign is folded into the pair before the final rounding rather
than negated afterward: negating a magnitude that was already rounded in the
active mode flips directed rounding, since
$-\,\mathrm{RU}(|v|) = \mathrm{RD}(-|v|)$.  For a positive exponent the binary
weight is then reattached in a single active-mode rounding: a power-of-two scale
when the result is normal (exact), and a direct rounding onto the subnormal grid
(lsb $2^{-1074}$) when it falls below the normal floor, so a tiny result does not
double-round.  For a negative exponent the pair is reciprocated and scaled, and a
result that is not finite, non-zero, and normal is handed back to the kernel for
the standard saturation and signaling.

For half-integer $y$ with $|2y| \le 2048$ (and $y$ not an integer, which the
integer path above already owns) the identity $x^{y} = (\sqrt{x})^{2y}$ is used.
The endpoints are handled directly: $y = \tfrac12$ returns the correctly-rounded
\texttt{sqrt\_gen}, and $y = -\tfrac12$ returns a correctly-rounded $1/\sqrt{x}$
obtained by pinning a faithful candidate $r$ with the \emph{exact} sign of
$r^2 x - 1$.  That residual is exact in every mode: $r$ and $x$ are doubles, so
$r^2$ is exact in two doubles via the FMA transform, each term times $x$ is exact
in two more, and the nine-term sum minus one is signed by an integer
fixed-point accumulation (a floating-point compensated sum would be exact only to
nearest).  The candidate is bracketed by walking with that sign and the active
mode selects the neighbor, with an exact midpoint test for round to nearest, so a
double-double $1/\sqrt{x}$ that would fall a directed-mode ULP short just past a
power of two is corrected.  Other half-integers raise a double-double $\sqrt{x}$
to the odd integer $2y$ by the same exponent-tracked square-and-multiply and
reattach the weight as above.  Any non-normal algebraic result is handed back to
the kernel so that the overflow, underflow, and subnormal semantics of the
...
correct-rounding in all four IEEE modes, including the subnormal scale branch and
the negative-base integer path discussed above, is validated empirically by the
MPFR and CORE-MATH all-mode campaigns over the generic runtime path; those
measurements are engineering evidence and are not part of the formal
certificates.
\end{remark}
