...
namespace ccm::showcase::ccmath_path
{
	...
	{
		if (raw == "public_default" || raw == "public")
		{
			return path::public_default;
		}
		if (raw == "generic_gen" || raw == "generic_runtime" || raw == "generic")
		{
			return path::generic_gen;
		}
		if (raw == "runtime_rt" || raw == "rt")
		{
			return path::runtime_rt;
		}
		if (raw == "runtime_simd" || raw == "simd")
		{
			return path::runtime_simd;
		}
		if (raw == "runtime_builtin" || raw == "builtin")
		{
			return path::runtime_builtin;
		}
		return std::nullopt;
	}
