...
#define REQUIRE_POWL_UNSUPPORTED_TIER()                                                                                            \
	...
	{                                                                                                                              \
		...
		if (format != ccm::config::LongDoubleFormat::IEEEBinary128 && format != ccm::config::LongDoubleFormat::Unknown)           \
		{                                                                                                                          \
...
TEST(PowlUnsupportedTier, PolicySelectsFallbackOrUnsupported)
{
	...
	if (ccm::config::reduced_precision_powl_fallback_enabled())
	{
		...
		const long double expected =
			static_cast<long double>(ccm::gen::pow_gen(static_cast<double>(base), static_cast<double>(exponent)));
