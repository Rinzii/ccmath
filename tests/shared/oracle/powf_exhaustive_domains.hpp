...
namespace ccm::test::oracle::powf_domains
{
	inline std::set<std::string_view> parse_domain_filter(const std::optional<std::string> & raw_domains)
	{
		...
		if (!raw_domains.has_value() || raw_domains->empty())
		{
			return domains;
		}
	...
	template <typename AddCaseFn> void add_mantissa_sweep(campaign_mode mode, AddCaseFn add_case, const std::set<std::string_view> & filter)
	{
		if (!domain_enabled(filter, "mantissa-sweep"))
		{
			return;
		}
	...
	template <typename AddCaseFn> void add_all_x_for_y(campaign_mode mode, AddCaseFn add_case, const std::set<std::string_view> & filter)
	{
		if (!domain_enabled(filter, "all-x-for-y"))
		{
			return;
		}
	...
	template <typename AddCaseFn> void add_all_y_for_x(campaign_mode mode, AddCaseFn add_case, const std::set<std::string_view> & filter)
	{
		if (!domain_enabled(filter, "all-y-for-x"))
		{
			return;
		}
	...
	template <typename AddCaseFn> void add_exponent_field_sweep(campaign_mode mode, AddCaseFn add_case, const std::set<std::string_view> & filter)
	{
		if (!domain_enabled(filter, "exponent-field-sweep"))
		{
			return;
		}
	...
	template <typename AddCaseFn> void add_subnormal_x(campaign_mode mode, AddCaseFn add_case, const std::set<std::string_view> & filter)
	{
		if (!domain_enabled(filter, "subnormal-x"))
		{
			return;
		}
	...
	template <typename AddCaseFn> void add_unit_interval(campaign_mode mode, AddCaseFn add_case, const std::set<std::string_view> & filter)
	{
		if (!domain_enabled(filter, "unit-interval"))
		{
			return;
		}
	...
	{
		if (!domain_enabled(filter, "negative-base"))
		{
			return;
		}
	...
	template <typename AddCaseFn> void add_overflow_threshold(AddCaseFn add_case, const std::set<std::string_view> & filter)
	{
		if (!domain_enabled(filter, "overflow-threshold"))
		{
			return;
		}
	...
	template <typename AddCaseFn> void add_underflow_threshold(AddCaseFn add_case, const std::set<std::string_view> & filter)
	{
		if (!domain_enabled(filter, "underflow-threshold"))
		{
			return;
		}
