...
namespace ccm::test::oracle::pow_mpfr
{
		...
		run_path_campaign<T>(
			...
			[&](run_summary<T>& path_summary)
			{
				for (int rounding_mode : rounding_modes)
				{
					...
					{
						if (path == validation_path::generic_modeled_domain && !is_modeled_generic_pow_case(test_case.base, test_case.exponent)) { continue; }
						(void)evaluate_case(
							test_case,
			...
			{ return make_campaign_report<T>(pow_path::path_name(path), path, mode, path_summary, seed, elapsed_ms, domains_covered, domains_skipped); },
	...
	inline int run_campaign(int argc, char** argv)
	{
		...
		std::uint64_t const target_ulp = parse_option_or<std::uint64_t>(
			option_value(argc, argv, "--target-ulp="), [](std::string const& value) { return static_cast<std::uint64_t>(std::stoull(value)); }, 0);
		...
		std::vector<std::string> domains_covered;
		std::vector<std::string> domains_skipped;
		if constexpr (std::is_same_v<T, double>) { domains_covered = { std::string(traits<T>::corpus_label) }; }
