...
{
	template <typename T> using pow_case = ccm::test::oracle::pow_case<T>;
	...
	template <typename T> inline void add_overflow_boundary(std::vector<pow_case<T>> & cases)
	{
	...
	template <typename T> inline void add_underflow_boundary(std::vector<pow_case<T>> & cases)
	{
		if constexpr (std::is_same_v<T, float>)
		{
	...
	template <typename T> inline void add_subnormal_output(std::vector<pow_case<T>> & cases, std::uint64_t seed, std::size_t count)
	{
	...
	template <typename T> inline void add_table_boundary(std::vector<pow_case<T>> & cases)
	{
	...
	template <typename T> inline void add_field_mutation(std::string_view mode, std::uint64_t seed, std::size_t count, std::vector<pow_case<T>> & cases)
	{
		std::mt19937_64 rng(seed);
		for (std::size_t i = 0; i < count; ++i)
		{
	...
	template <typename T> inline std::vector<pow_case<T>> build_search_cases(std::string_view mode, std::uint64_t seed, std::size_t count)
	{
