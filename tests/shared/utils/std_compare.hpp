...
namespace ccm::test
{
	template <typename Actual, typename Expected> void ExpectSameAsStd(Actual actual, Expected expected, int64_t max_ulp = kMaxAllowedUlp)
	{
	...
	template <typename T, typename CcmFn, typename StdFn> void ExpectUnaryMatchesStd(T input, CcmFn ccm_fn, StdFn std_fn, int64_t max_ulp = kMaxAllowedUlp)
	{
		SCOPED_TRACE(input);
		ExpectSameAsStd(ccm_fn(input), std_fn(input), max_ulp);
	}

	template <typename T, typename CcmFn, typename StdFn> void ExpectBinaryMatchesStd(T x, T y, CcmFn ccm_fn, StdFn std_fn, int64_t max_ulp = kMaxAllowedUlp)
	{
		SCOPED_TRACE(x);
		SCOPED_TRACE(y);
	...
	template <typename T> void ExpectFrexpMatchesStd(T input)
	{
		int ccm_exp{};
		int std_exp{};
	...
	template <typename T> void ExpectModfMatchesStd(T input)
	{
		T ccm_int{};
		T std_int{};
	...
	template <typename T> void ExpectRemquoMatchesStd(T x, T y)
	{
		int ccm_quo{};
		int std_quo{};
