...
namespace
{
	template <typename T> void SmokePath(ccm::test::pow_path::validation_path path)
	{
		const auto support = ccm::test::pow_path::path_is_supported<T>(path);
		if (!support.supported)
		{
			GTEST_SKIP() << support.skip_reason;
		}
		...
		for (T base : bases)
		{
			...
			{
				if (path == ccm::test::pow_path::validation_path::generic_modeled_domain && !(base > T{}))
				{
					continue;
				}
