...
namespace ccm::test::oracle
{
	...
	namespace powl_path_detail
	{
		...
		{
			if (exp == 0.0L) { return true; }
			if (base == 1.0L) { return true; }
			if (ccm::isnan(base) || ccm::isnan(exp)) { return true; }

			using FPBits_t = support::fp::FPBits<long double>;
			const FPBits_t base_bits(base);
			const FPBits_t exp_bits(exp);

			if (base_bits.is_zero()) { return true; }
			if (base_bits.is_inf()) { return true; }
			if (exp_bits.is_inf()) { return true; }
			if (base_bits.is_neg() && !gen::internal::powl_bits::is_integer(exp)) { return true; }
			return false;
		}
		...
		{
			if (is_ld80_special_case_route(base, exp)) { return PowlImplementationPath::Ld80SpecialCase; }
			if (is_ld80_bounded_integer_route(base, exp)) { return PowlImplementationPath::Ld80BoundedInteger; }
