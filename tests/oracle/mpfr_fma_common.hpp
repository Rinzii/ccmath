...
namespace ccm::test::oracle::fma_oracle
{
	...
	inline T invoke(validation_path path, T x, T y, T z)
	{
		...
		case validation_path::public_default:
			...
			else { return ccm::fma(ccm::test::runtime_value(x), ccm::test::runtime_value(y), ccm::test::runtime_value(z)); }
	...
	inline int run_campaign(int argc, char ** argv, std::string_view function_name, mpfr_prec_t default_precision)
	{
		...
		const std::uint64_t seed = parse_option_or<std::uint64_t>(
			option_value(argc, argv, "--seed="), [](const std::string & value) { return std::stoull(value); }, 0xF6A600D5EEDULL);
		...
		const mpfr_prec_t precision = parse_option_or<mpfr_prec_t>(
			option_value(argc, argv, "--oracle-precision="), [](const std::string & value) { return static_cast<mpfr_prec_t>(std::stoul(value)); }, default_precision);
		...
		std::cout << "mpfr " << function_name << " campaign mode=" << mode_name(mode) << " fma=" << fma_status()
				  << " configuration=" << fma_configuration_name() << " rounding_modes=" << rounding_modes.size()
				  << " cases=" << cases.size() << '\n';
		...
		for (validation_path path : paths)
		{
			...
			std::cout << "path=" << path_name(path) << " configuration=" << fma_configuration_name()
					  << " rounding_modes=" << rounding_modes.size() << " cases=" << summary.case_count
					  << " mpfr_policy_mismatches=" << summary.mpfr_policy_mismatch_count << " max_ulp=" << summary.max_observed_ulp
					  << " failures=" << summary.failure_count << " elapsed_ms=" << elapsed.count() << '\n';
		}
