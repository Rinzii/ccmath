...
namespace ccm::test::oracle::powf_domains
{
	...
	void add_all_x_for_y(campaign_mode mode, AddCaseFn add_case, const std::set<std::string_view>& filter)
	{
		...
		{
			for (std::uint32_t bits = 0; bits < 0x7f800000u; bits += step) { add_case(ccm::support::bit_cast<float>(bits), exponent, "all-x-for-selected-y"); }
		}
	}
	...
	void add_all_y_for_x(campaign_mode mode, AddCaseFn add_case, const std::set<std::string_view>& filter)
	{
		...
		{
			for (std::uint32_t bits = 0; bits < 0x7f800000u; bits += step) { add_case(base, ccm::support::bit_cast<float>(bits), "all-y-for-selected-x"); }
		}
	}
	...
	void add_negative_base([[maybe_unused]] campaign_mode mode, AddCaseFn add_case, const std::set<std::string_view>& filter)
	{
		...
		for (float exponent : { 0.5F, 1.5F, 3.5F, std::nextafter(1.0F, 2.0F) }) { add_case(-2.0F, exponent, "negative-base non-integer boundary campaign"); }
	}
	...
	void add_overflow_threshold(AddCaseFn add_case, const std::set<std::string_view>& filter)
	{
		...
		for (float exponent : { 126.0F, 127.0F, 128.0F, 129.0F }) { add_case(2.0F, exponent, "overflow-threshold campaign"); }
	}
	...
	void add_underflow_threshold(AddCaseFn add_case, const std::set<std::string_view>& filter)
	{
		...
		for (float exponent : { -150.0F, -151.0F, -152.0F }) { add_case(2.0F, exponent, "underflow-threshold campaign"); }
	}
} // namespace ccm::test::oracle::powf_domains
