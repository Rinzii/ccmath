...
namespace ccm::test::oracle::adversarial
{
	...
	inline std::vector<pow_case<T>> build_search_cases(std::string_view mode, std::uint64_t seed, std::size_t count)
	{
		...
		if (mode == "overflow-boundary-search" || mode == "overflow-boundary") { add_overflow_boundary(cases); return cases; }
		if (mode == "underflow-boundary-search" || mode == "underflow-boundary") { add_underflow_boundary(cases); return cases; }
		if (mode == "negative-base-integer-boundary-search" || mode == "negative-base-parity") { add_negative_base_parity(cases); return cases; }
		...
		if (mode == "near-one") { add_near_one(cases); return cases; }
		if (mode == "near-negative-one") { add_near_negative_one(cases); return cases; }
		if (mode == "near-zero") { add_near_zero(cases); return cases; }
		if (mode == "near-power-of-two") { add_near_power_of_two(cases); return cases; }
		if (mode == "near-integer-exponent") { add_near_integer_exponent(cases); return cases; }
		if (mode == "subnormal-output") { add_subnormal_output(cases, seed, count); return cases; }
