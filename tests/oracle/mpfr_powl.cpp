...
namespace
{
									...
									mpfr_prec_t precision)
	{
		...
		{
			accumulate_report(report, evaluate_powl_case(test_case, precision, 4));
		}

		std::cout << "powl corpus=" << corpus_name << " format=" << ccm::config::long_double_format_name(format)
				  << " fallback_enabled=" << (report.fallback_enabled ? "yes" : "no")
...
int main(int argc, char ** argv)
{
	...
	{
		reports.push_back(run_corpus("ld64_conservative", build_ld64_cases(), format, precision));
	}
	else if (format == ccm::config::LongDoubleFormat::X87Extended)
	{
		reports.push_back(run_corpus("ld80_special_cases", build_ld80_special_cases(), format, precision));
		reports.push_back(run_corpus("ld80_bounded_integer", build_ld80_bounded_integer_cases(), format, precision));
		reports.push_back(run_corpus("ld80_general_finite", build_ld80_general_finite_cases(), format, precision));
	}
	else
	{
