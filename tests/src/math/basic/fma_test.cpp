...
namespace
{
	...
	constexpr float kFloatResidual = ccm::fmaf(0x1.fffep-1F, 0x1.fffep-1F, -(0x1.fffep-1F * 0x1.fffep-1F));
#if CCMATH_SUPPORTS_DEEP_CONSTEXPR
	constexpr double kDoubleResidual = ccm::fma(0x1.ffffffffffep-1, 0x1.ffffffffffep-1, -(0x1.ffffffffffep-1 * 0x1.ffffffffffep-1));
#endif
	...
	static_assert(ccm::support::bit_cast<std::uint32_t>(ccm::support::fp::public_fma<float>(kKillerX, kKillerX, kKillerZ)) == kKillerCorrect,
				  "constexpr public_fma<float> must round the fused result once, not twice");
	static_assert(ccm::support::bit_cast<std::uint32_t>(ccm::support::fp::generic_fma<float>(kKillerX, kKillerX, kKillerZ)) == kKillerCorrect,
				  "constexpr generic_fma<float> must round the fused result once, not twice");
	static_assert(ccm::support::bit_cast<std::uint32_t>(ccm::support::fp::fma_internal::fixed_fma<float, true>(kKillerX, kKillerX, kKillerZ)) == kKillerCorrect,
...
TEST(CcmathBasicTests, FmaMatchesStdStructuredFloatCases)
{
	...
	{
		ccm::test::ExpectTernaryMatchesStd(input.x, input.y, input.z, ccm::fmaf, static_cast<float (*)(float, float, float)>(std::fma));
	}
}
...
TEST(CcmathBasicTests, FmaMatchesStdStructuredDoubleCases)
{
	for (const auto & input : ccm::test::samples::kFmaDoubleCases)
	{
		ccm::test::ExpectTernaryMatchesStd(
			input.x, input.y, input.z, static_cast<double (*)(double, double, double)>(ccm::fma), static_cast<double (*)(double, double, double)>(std::fma));
	}
}

TEST(CcmathBasicTests, FmaMatchesStdLongDoubleSmokeCases)
{
	ccm::test::ExpectTernaryMatchesStd(1.0L, 2.0L, 3.0L, ccm::fmal, static_cast<long double (*)(long double, long double, long double)>(std::fma));
	ccm::test::ExpectTernaryMatchesStd(
		0x1.fffffffffffffp-1L, 0x1.fffffffffffffp-1L, -0x1.ep-2L, ccm::fmal, static_cast<long double (*)(long double, long double, long double)>(std::fma));
}
