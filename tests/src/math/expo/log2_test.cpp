...
TEST(CcmathExponentialTests, Log2)
{
	...
	{
		ccm::test::ExpectUnaryMatchesStd(input, ccm::log2<double>, static_cast<double (*)(double)>(std::log2));
	}
	...
	{
		ccm::test::ExpectUnaryMatchesStd(input, ccm::log2<float>, static_cast<float (*)(float)>(std::log2));
	}
