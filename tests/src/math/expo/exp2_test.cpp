...
TEST(CcmathExponentialTests, Exp2_Double_ValidInput)
{
	...
	{
		ccm::test::ExpectUnaryMatchesStd(input, ccm::exp2<double>, static_cast<double (*)(double)>(std::exp2));
	}
...
TEST(CcmathExponentialTests, Exp2_Float)
{
	...
	{
		ccm::test::ExpectUnaryMatchesStd(input, ccm::exp2<float>, static_cast<float (*)(float)>(std::exp2));
	}
}
