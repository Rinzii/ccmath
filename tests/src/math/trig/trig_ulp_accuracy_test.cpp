...
namespace
{
	template <typename T, typename CcmFn, typename StdFn> void ExpectTanAwayFromPole(T input, CcmFn ccm_fn, StdFn std_fn, T pole_guard)
	{
		if (std::abs(std::cos(input)) < pole_guard)
		{
			return;
		}
