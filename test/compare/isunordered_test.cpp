...
TEST(CcmathCompareTests, IsUnordered)
{
	...
	EXPECT_EQ(ccm::isunordered(std::numeric_limits<double>::quiet_NaN(), std::numeric_limits<double>::quiet_NaN()),
			  std::isunordered(std::numeric_limits<double>::quiet_NaN(), std::numeric_limits<double>::quiet_NaN()));
}
